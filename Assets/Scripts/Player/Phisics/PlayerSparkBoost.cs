...
public class PlayerSparkBoost : MonoBehaviour
{
    ...
    [Header("Dash Combat Mode")]
    public bool dashUsesBounceCombat = false;

    [Tooltip("Daño del dash cuando dashUsesBounceCombat = true.")]
    public int dashCombatDamage = 20;
    ...
    // Dash combat anti-multi-hit
    private readonly HashSet<Collider2D> impactedThisDash = new HashSet<Collider2D>();
    private readonly HashSet<Collider2D> piercedThisDash = new HashSet<Collider2D>();
    ...
    private void ApplyResidualAbsorb()
    {
        ...
        rb.linearVelocity = new Vector2(0f, v.y);   // NO toques la Y
    ...
    private void TriggerDash()
    {
        ...
        // Limpia impactos/pierces por dash (si no, arrastras estado viejo)
        impactedThisDash.Clear();
        piercedThisDash.Clear();
    ...
    private void DashStep(float dt)
    {
        ...
        while (remaining > 0f && safety++ < SAFETY_MAX)
        {
            ...
            if (pickup != null && pickupDist <= remaining + dashSkin)
            {
                if (PickupUsable(pickup))
                {
                    ...
                    // Si este pickup define combate, lo aplicamos para el siguiente Spark
        ...
        // =======================
        // DASH COMBAT (pierce like BounceAttack)
        // =======================
        if (dashUsesBounceCombat && dashCombatDamage > 0)
        {
            bool pierced = ApplyDashPiercingImpact(hit.collider, dir, dashCombatDamage);

            // Si pierceó (rompió / receiver lo permitió), seguimos recto (NO reflect)
            if (pierced)
            {
                float microPush = Mathf.Max(0.01f, dashSkin * 2f);
                float pushed = Mathf.Min(microPush, remaining);
                pos += dir * pushed;
                remaining -= pushed;

                // seguimos el while con el mismo dir
                continue;
            }
        }

// Si no pierceó (o no está en combat), comportamiento actual:
...
remaining -= nudged;
    ...
    private void ConfigurePreviewAuto()
    {
        ...
        sparkPreviewLine.numCapVertices = 0;
        sparkPreviewLine.numCornerVertices = 0;
    ...
    private Vector2 GetColliderSupportOffset(Vector2 dir)
    {
        ...
        if (boxCol != null)
        {
            ...
            // CLAVE: elegir el lado dominante (horizontal vs vertical)
            // para no irte al CORNER y evitar los "picos".
            if (Mathf.Abs(d.x) >= Mathf.Abs(d.y))
            {
                // movimiento más horizontal -> salir por el centro del lado izq/der
                return offW + new Vector2(Mathf.Sign(d.x) * e.x, 0f);
            }
            else
            {
                // movimiento más vertical -> salir por el centro del lado arriba/abajo
                return offW + new Vector2(0f, Mathf.Sign(d.y) * e.y);
            }
        }
...
// ======================================================================
        ...
        // IMPORTANTE: empujamos TODOS los puntos al borde del collider,
        // no solo el primero y el último (eso crea los “ganchos/picos”).
        for (int i = 0; i < nPts; i++)
        {
            Vector2 d;

            if (i < dirPts.Count) d = dirPts[i];
            else d = dir0; // fallback

            if (d.sqrMagnitude < 0.000001f) d = dir0;

            renderPts[i] = (Vector3)(centerPts[i] + GetColliderSupportOffset(d));
}
    ...
    private bool ApplyDashPiercingImpact(Collider2D col, Vector2 direction, float incomingDamage)
    {
        if (col == null) return false;

        // Si ya lo pierceaste en este dash, compórtate como si no existiera (evita pegotes)
        if (piercedThisDash.Contains(col))
            return true;

        // Evita multi-hit por micro-colisiones
        if (impactedThisDash.Contains(col))
            return false;

        var receiverPierce = col.GetComponentInParent<IPiercingBounceReceiver>();
        if (receiverPierce != null)
        {
            impactedThisDash.Add(col);

            var impact = new BounceImpactData((int)Mathf.Ceil(incomingDamage), direction, gameObject);

            bool pierced = receiverPierce.ApplyPiercingBounce(impact, incomingDamage, out float _rem);

            if (pierced)
                piercedThisDash.Add(col);
        ...
        if (receiver != null)
        {
            receiver.ReceiveBounceImpact(new BounceImpactData((int)Mathf.Ceil(incomingDamage), direction, gameObject));
            impactedThisDash.Add(col);
        }
